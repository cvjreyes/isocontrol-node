...
const update = async (req, res) => {
  try {
    ...
    if (extension == "pdf" && !cl) {
      const folders = [
        "./app/storage/isoctrl/design",
        "./app/storage/isoctrl/issuer",
        "./app/storage/isoctrl/lde",
        "./app/storage/isoctrl/materials",
        "./app/storage/isoctrl/stress",
        "./app/storage/isoctrl/supports",
      ];
      for (let i = 0; i < folders.length; i++) {
        let path = null;
        if (cl) {
          path =
            folders[i] + "/" + req.file.originalname.split(".").slice(0, -1);
          path = path.slice(0, -3);
        } else {
          path =
            folders[i] + "/" + req.file.originalname.split(".").slice(0, -1);
...
const getListFiles = (req, res) => {
  ...
  if (process.env.NODE_PROGRESS === "1") {
    ...
    if (tab === "On hold") {
      sql.query(
        "SELECT * FROM misoctrls WHERE onhold = 1",
        [tab],
        (err, results) => {
          res.json({
            rows: results,
      ...
      sql.query(
        "SELECT * FROM misoctrls WHERE misoctrls.to = ?",
        [tab],
        (err, results) => {
          res.json({
            rows: results,
...
const piStatus = (req, res) => {
  ...
  sql.query(
    "SELECT spo, sit FROM misoctrls WHERE filename = ?",
    fileName,
    (err, results) => {
      if (!results[0]) {
...
const download = (req, res) => {
  ...
  let issued = false;
  if (
    master.substring(master.length - 3, master.length) === "-0" ||
    master.substring(master.length - 3, master.length) === "-1" ||
    master.substring(master.length - 3, master.length) === "-2" ||
    master.substring(master.length - 3, master.length) === "-3"
  ) {
    issued = true;
  }
  ...
  sql.query(
    "SELECT isoid FROM misoctrls WHERE filename = ?",
    master,
    (err, results) => {
      if (!results[0]) {
        res.status(401);
        ...
        let where,
          path = null;
        sql.query(
          "SELECT issued, transmittal, issued_date FROM misoctrls WHERE filename = ?",
          master,
          (err, results) => {
            if (!results[0]) {
              res.status(401);
              ...
              if (results[0].issued != 1) {
                const folders = [
                  ...
                  "./app/storage/isoctrl/stress",
                  "./app/storage/isoctrl/supports",
                  "./app/storage/isoctrl/design/attach",
                  "./app/storage/isoctrl/issuer/attach",
                  "./app/storage/isoctrl/lde/attach",
                  "./app/storage/isoctrl/materials/attach",
                  "./app/storage/isoctrl/stress/attach",
                  "./app/storage/isoctrl/supports/attach",
                  "./app/storage/isoctrl/design/TRASH",
                  "./app/storage/isoctrl/issuer/TRASH",
                  "./app/storage/isoctrl/lde/TRASH",
                  "./app/storage/isoctrl/materials/TRASH",
                  "./app/storage/isoctrl/stress/TRASH",
                  "./app/storage/isoctrl/supports/TRASH",
                  "./app/storage/isoctrl/design/TRASH/tattach",
                  "./app/storage/isoctrl/issuer/TRASH/tattach",
                  "./app/storage/isoctrl/lde/TRASH/tattach",
                  "./app/storage/isoctrl/materials/TRASH/tattach",
                  "./app/storage/isoctrl/stress/TRASH/tattach",
                  "./app/storage/isoctrl/supports/TRASH/tattach",
                  "./app/storage/isoctrl/design/HOLD",
                  "./app/storage/isoctrl/issuer/HOLD",
                  "./app/storage/isoctrl/lde/HOLD",
                  "./app/storage/isoctrl/materials/HOLD",
                  "./app/storage/isoctrl/stress/HOLD",
                  "./app/storage/isoctrl/supports/HOLD",
                  "./app/storage/isoctrl/design/HOLD/hattach",
                  "./app/storage/isoctrl/issuer/HOLD/hattach",
                  "./app/storage/isoctrl/lde/HOLD/hattach",
                  "./app/storage/isoctrl/materials/HOLD/hattach",
                  "./app/storage/isoctrl/stress/HOLD/hattach",
                  "./app/storage/isoctrl/supports/HOLD/hattach",
                ];
                ...
                res.download(
                  "./app/storage/isoctrl/lde/transmittals/" +
                    trn +
                    "/" +
                    date +
                    "/" +
...
const getAttach = (req, res) => {
  ...
  sql.query(
    "SELECT transmittal, issued_date FROM misoctrls WHERE filename = ?",
    fileName,
    (err, results) => {
      if (!results[0].transmittal) {
        ...
        fs.readdir(origin_attach_path, (err, files) => {
          if (files) {
            ...
            if (fs.existsSync(origin_cl_path)) {
              allFiles.push(
                fileName.split(".").slice(0, -1).join(".") + "-CL.pdf"
              );
            }

            if (fs.existsSync(origin_proc_path)) {
              allFiles.push(
                fileName.split(".").slice(0, -1).join(".") + "-PROC.pdf"
              );
            }

            if (fs.existsSync(origin_inst_path)) {
              allFiles.push(
                fileName.split(".").slice(0, -1).join(".") + "-INST.pdf"
              );
            }
          }
        ...
        folders = [
          "./app/storage/isoctrl/lde/transmittals/" +
            results[0].transmittal +
            "/" +
            results[0].issued_date,
        ];
        ...
        let origin_cl_path =
          where + "/" + fileName.split(".").slice(0, -1).join(".") + "-CL.pdf";
        let origin_proc_path =
          where +
          "/" +
          fileName.split(".").slice(0, -1).join(".") +
          "-PROC.pdf";
        let origin_inst_path =
          where +
          "/" +
          fileName.split(".").slice(0, -1).join(".") +
          "-INST.pdf";
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        sql.query(
          "INSERT INTO hisoctrls (filename, revision, claimed, spo, sit, `from`, `to`, comments, user, role) VALUES (?,?,?,?,?,?,?,?,?,?)",
          [
            req.body.fileName,
            0,
            1,
          ...
          (err, results) => {
            if (err) {
              ...
              if (process.env.NODE_PROGRESS == "1") {
                ...
                sql.query(
                  "SELECT tpipes_id FROM dpipes_view WHERE isoid COLLATE utf8mb4_unicode_ci = ?",
                  [req.body.fileName.split(".").slice(0, -1)],
                  (err, results) => {
                    if (!results[0]) {
                      res.status(401);
                      ...
                      const q =
                        "SELECT " +
                        type +
                        " FROM ppipes WHERE level = ? AND tpipes_id = ?";
                ...
                sql.query(
                  "INSERT INTO misoctrls (filename, isoid, revision, claimed, spo, sit, `from`, `to`, comments, user, role, progress) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                  [
                    req.body.fileName,
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        username = results[0].name;
        sql.query(
          "SELECT * FROM hisoctrls WHERE filename = ?",
          [fileName],
          (err, results) => {
            if (!results[0]) {
              ...
              sql.query(
                "INSERT INTO hisoctrls (filename, revision, spo, sit, `from`, `to`, comments, user, role) VALUES (?,?,?,?,?,?,?,?,?)",
                [
                  fileName,
                  last.revision,
                  last.spo,
...
const updateStatus = async (req, res) => {
  let designR0 = 0,
    ...
    modelCount = 0;
  sql.query(
    "SELECT `to`, issued, revision FROM misoctrls WHERE (revision = 0 OR revision = 1) AND (onhold != 1 || onhold IS NULL)",
    (err, results) => {
      if (!results[0]) {
        results = [];
      }
      for (let i = 0; i < results.length; i++) {
        if (results[i].to == "Design" && results[i].revision == 0) {
          ...
          (results[i].to == "Stress" || results[i].to == "stress") &&
          results[i].revision == 0
        ) {
          stressR0 += 1;
          ...
          results[i].to == "LDE/Isocontrol" &&
          results[i].revision == 0 &&
          (results[i].issued == null || results[i].issued == 0)
        ) {
          toIssueR0 += 1;
        } else if (
          results[i].to == "LDE/Isocontrol" &&
          results[i].issued == 1 &&
          results[i].revision == 1
        ) {
          issuedR0 += 1;
        }
      }
      ...
      totalR0 =
        ...
        issuedR0;
      sql.query(
        "SELECT `to`,issued, revision FROM misoctrls WHERE (revision = 1 OR revision = 2) AND (onhold != 1 || onhold IS NULL)",
        (err, results) => {
          if (!results[0]) {
            results = [];
          }
          for (let i = 0; i < results.length; i++) {
            if (results[i].to == "Design" && results[i].revision == 1) {
              ...
              results[i].to == "Supports" &&
              results[i].revision == 1
            ) {
              supportsR1 += 1;
            } else if (
              results[i].to == "Materials" &&
              results[i].revision == 1
            ) {
              materialsR1 += 1;
          ...
          sql.query(
            "SELECT `to`, issued, revision FROM misoctrls WHERE (revision = 2 OR revision = 3) AND (onhold != 1 || onhold IS NULL)",
            (err, results) => {
              if (!results[0]) {
                results = [];
              }
              ...
              totalR2 =
                ...
                issuedR2;
              sql.query(
                "SELECT `to` FROM misoctrls WHERE onhold = 1",
                (err, results) => {
                  if (!results[0]) {
                    results = [];
                  }
                  ...
                  totalHold =
                    ...
                    toIssueHold;
                  sql.query(
                    "SELECT `from`, issued FROM misoctrls WHERE `to` = ?",
                    ["Recycle bin"],
                    (err, results) => {
                      if (!results[0]) {
                        results = [];
                      ...
                      for (let i = 0; i < results.length; i++) {
                        if (results[i].from == "Design") {
                          ...
                          results[i].from == "LDE/Isocontrol" &&
                          results[i].issued == null
                        ) {
                          toIssueDeleted += 1;
                        } else if (
                          results[i].from == "LDE/Isocontrol" &&
                      ...
                      sql.query(
                        ...
                        (err, results) => {
                          ...
                          totalDeleted =
                            ...
                            issuedDeleted;
                          designStock =
                            designR0 + designR1 + designR2 + designHold;
                          stressStock =
                            stressR0 + stressR1 + stressR2 + stressHold;
                          supportsStock =
                            supportsR0 + supportsR1 + supportsR2 + supportsHold;
                          materialsStock =
                            materialsR0 +
                            materialsR1 +
                            materialsR2 +
                            materialsHold;
                          issuerStock =
                            issuerR0 + issuerR1 + issuerR2 + issuerHold;
                          toIssueStock =
                            toIssueR0 + toIssueR1 + toIssueR2 + toIssueHold;
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        username = results[0].name;
        sql.query(
          "SELECT * FROM misoctrls WHERE filename = ?",
          [fileName],
          (err, results) => {
            if (!results[0]) {
              res.status(401).send("No files found");
            } else if (
              (results[0].deleted == 0 || results[0].deleted == null) &&
              (results[0].onhold == 0 || results[0].onhold == null)
            ) {
              ...
              sql.query(
                "INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, `from`, `to`, comments, user, role) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                [
                  fileName,
                  results[0].revision,
                  results[0].spo,
                    ...
                    sql.query(
                      ...
                      (err, results) => {
                        if (err) {
                          ...
                          let masterName = req.body.fileName
                            .split(".")
                            .slice(0, -1);
                          let origin_path,
                            destiny_path,
                          ...
                          if (origin == "Recycle bin") {
                            ...
                            origin_attach_path =
                              ...
                              "/TRASH/tattach/";
                            destiny_attach_path =
                              "./app/storage/isoctrl/" + destiny + "/attach/";
                            origin_cl_path =
                              "./app/storage/isoctrl/" +
                              destiny +
                              "/TRASH/tattach/" +
                            ...
                            fs.readdir(origin_attach_path, (err, files) => {
                              files.forEach((file) => {
                                ...
                                if (
                                  ...
                                  String(attachName).trim()
                                ) {
                                  fs.rename(
                                    origin_attach_path + file,
                                    destiny_attach_path + file,
                                    function (err) {
                                      if (err) throw err;
                                    }
                                  );
      ...
      (err, results) => {
        if (!results[0]) {
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            if (results[i].to == "Design") {
              if (
                results[i].verifydesign == 1 ||
                results[i].role == "DesignLead"
              ) {
...
const historyFiles = (req, res) => {
  sql.query(
    "SELECT * FROM hisoctrls ORDER BY created_at DESC",
    (err, results) => {
      if (!results[0]) {
        res.status(401).send("No files found");
      } else {
...
const modelled = (req, res) => {
  sql.query(
    "SELECT tag, dpipes_view.isoid, code, blocked FROM dpipes_view RIGHT JOIN tpipes ON tpipes.id = dpipes_view.tpipes_id LEFT JOIN misoctrls ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid group by dpipes_view.isoid, blocked",
    (err, results) => {
      if (!results[0]) {
        res.status(401);
      } else {
        ...
        sql.query(
          ...
          (err, results) => {
            if (err) {
              ...
              sql.query(
                "INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, spoclaimed, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                  fileName,
                  file.revision,
                  nextProcess,
        ...
        sql.query(
          ...
          (err, results) => {
            if (err) {
              ...
              sql.query(
                "INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, sitclaimed, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                [
                  fileName,
                  file.revision,
                  file.spo,
...
const cancelProc = (req, res) => {
  ...
  let prev = 0;
  sql.query(
    "SELECT `from`,`to`, id, user, role FROM hisoctrls WHERE filename = ? AND role = ? ORDER BY id DESC LIMIT 1",
    [fileName, "Process"],
    (err, results) => {
      if (!results[0]) {
        prev = 0;
      ...
      sql.query(
        ...
        (err, results) => {
          if (!results[0]) {
            ...
            sql.query(
              "INSERT INTO hisoctrls (filename, spo, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?)",
              [
                fileName,
                prev,
                "Cancelled PRO",
      ...
      sql.query(
        "UPDATE misoctrls SET spo = ? WHERE filename = ?",
        [prev, fileName],
        (err, results) => {
          if (err) {
            res.status(401);
...
const cancelInst = (req, res) => {
  ...
  let prev = 0;
  sql.query(
    "SELECT `from` FROM hisoctrls WHERE filename = ? AND role = ? ORDER BY id DESC LIMIT 1",
    [fileName, "Instrument"],
    (err, results) => {
      if (!results[0]) {
        prev = 0;
      ...
      sql.query(
        ...
        (err, results) => {
          if (!results[0]) {
            ...
            sql.query(
              "INSERT INTO hisoctrls (filename, sit, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?)",
              [
                fileName,
                prev,
                "Cancelled PRO",
      ...
      sql.query(
        "UPDATE misoctrls SET sit = ? WHERE filename = ?",
        [prev, fileName],
        (err, results) => {
          if (err) {
            res.status(401);
...
const filesProcInst = (req, res) => {
  ...
  if (type == "Process") {
    sql.query(
      "SELECT * FROM misoctrls WHERE spo = 1 OR spo = 4 or spo = 5",
      (err, results) => {
        if (err) {
          res.status(401).send("No files found");
        } else {
    ...
    sql.query(
      "SELECT * FROM misoctrls WHERE sit = 1 OR sit = 4 or sit = 5",
      (err, results) => {
        if (err) {
          res.status(401).send("No files found");
        } else {
...
const uploadProc = async (req, res) => {
  ...
  if (req.file == undefined) {
    ...
    if (where.includes("HOLD")) {
      fs.rename(
        where + "/hattach/" + req.file.originalname,
        where +
          "/hattach/" +
          req.file.originalname.split(".").slice(0, -1).join(".") +
          "-PROC.pdf",
      ...
      fs.rename(
        where + "/attach/" + req.file.originalname,
        where +
          "/attach/" +
          req.file.originalname.split(".").slice(0, -1).join(".") +
          "-PROC.pdf",
  ...
  if (
    ...
    for (let i = 1; i < req.body.length; i++) {
      if (
        ...
        req.body[i][1] != null &&
        req.body[i][1] != "" &&
        !req.body[i][1].includes("/") &&
        !req.body[i][1].includes("=") &&
        !req.body[i][2] != null
      ) {
        sql.query(
          "SELECT id FROM areas WHERE name = ?",
          [req.body[i][area_index]],
          (err, results) => {
            let areaid = null;
            if (results[0]) {
              areaid = results[0].id;
            ...
            if (process.env.NODE_MMDN == 1) {
              sql.query(
                "SELECT id FROM diameters WHERE nps = ?",
                ...
                (err, results) => {
                  if (!results[0]) {
                    ...
                    let calc_notes = 0;
                    if (
                      req.body[i][calc_index] != "" &&
                      req.body[i][calc_index] != null
                    ) {
                      calc_notes = 1;
                    }
                    ...
                    sql.query(
                      "INSERT INTO dpipes(area_id, tag, diameter_id, calc_notes, tpipes_id) VALUES (?,?,?,?,?)",
                      [
                        areaid,
                        req.body[i][tag_index],
                        diameterid,
                        calc_notes,
                        tl,
                      ],
              ...
              sql.query(
                "SELECT id FROM diameters WHERE dn = ?",
                [req.body[i][diameter_index]],
                (err, results) => {
                  if (!results[0]) {
                    console.log(
                    ...
                    if (calc_notes == 0) {
                      ...
                      if (req.body[i][diameter_index] < 2.0) {
                        tl = 1;
                      } else {
                        tl = 2;
                      }
...
const checkPipe = async (req, res) => {
  ...
  sql.query(
    "SELECT * FROM dpipes_view WHERE isoid COLLATE utf8mb4_unicode_ci = ?",
    [fileName],
    (err, results) => {
      if (!results[0]) {
        res
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0] && process.env.NODE_PROGRESS == "1") {
        sql.query(
          "UPDATE misoctrls SET blocked = 1 WHERE filename = ?",
          [fileName],
          (err, results) => {
        ...
        sql.query(
          "SELECT isoid, revision FROM misoctrls WHERE filename = ?",
          [fileName],
          (err, results) => {
            if (!results[0]) {
                    ...
                    sql.query(
                      ...
                      (err, results) => {
                        if (!results[0]) {
                          ...
                          sql.query(
                            "INSERT INTO hisoctrls (filename, revision, spo, sit, issued, transmittal, issued_date, deleted, onhold, spoclaimed, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                            [
                              fileName,
                              revision,
                              last.spo,
                                  ...
                                  (err, results) => {
                                    if (err) {
                                      ...
                                      if (process.env.NODE_PROGRESS == "0") {
                                        sql.query(
                                          "UPDATE misoctrls SET revision = ?, claimed = 0, issued = 1, transmittal = ?, issued_date = ?, user = ?, role = ? WHERE filename = ?",
                                          [
                                            revision + 1,
                                            transmittal,
                                            date,
                                              ...
                                              sql.query(
                                                ...
                                                (err, results) => {
                                                  if (!results[0]) {
                                                    ...
                                                    sql.query(
                                                      "UPDATE misoctrls SET revision = ?, claimed = 0, issued = 1, user = ?, role = ?, progress = ?, realprogress = ?, transmittal = ?, issued_date = ?, max_tray = ? WHERE filename = ?",
                                                      [
                                                        revision + 1,
                                                        "None",
                                                        null,
                                                      ...
                                                      (err, results) => {
                                                        if (err) {
                                                          ...
                                                          sql.query(
                                                            "SELECT bypass.id, bstatus_id FROM bypass LEFT JOIN misoctrls ON bypass.misoctrls_id = misoctrls.id WHERE misoctrls.isoid COLLATE utf8mb4_unicode_ci = ?",
                                                            [isoid],
                                                            (err, results) => {
                                                              if (!results[0]) {
                                                                res
                                                                ...
                                                                for (
                                                                  ...
                                                                  sql.query(
                                                                    "UPDATE bypass SET bstatus_id = ? WHERE id = ?",
                                                                    [
                                                                      closed,
                                                                      results[i]
                                                                        .id,
                  ...
                  sql.query(
                    ...
                    (err, results) => {
                      if (results[0].requested !== null) {
                        ...
                        sql.query(
                          "UPDATE misoctrls SET requested = 1  WHERE filename = ?",
                          [fileName],
                          (err, results) => {
                            if (err) {
                              console.log("error: ", err);
                ...
                sql.query(
                  ...
                  (err, results) => {
                    if (!results[0]) {
                      ...
                      username = results[0].name;
                      sql.query(
                        "SELECT id, revision FROM misoctrls WHERE filename = ?",
                        [fileName],
                        (err, results) => {
                          if (!results[0]) {
                            res.status(401).send("File not found");
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        sql.query(
          "SELECT `to` FROM misoctrls WHERE filename = ?",
          [oldName],
          (err, results) => {
            if (!results[0]) {
              res.status(401);
                ...
                (err, results) => {
                  if (err) {
                    ...
                    sql.query(
                      ...
                      [newName, newName.split(".").slice(0, -1), oldName],
                      (err, results) => {
                        if (err) {
                          res.status(401);
                        } else {
                          ...
                          destiny_path =
                            ...
                            newName;
                          origin_attach_path =
                            "./app/storage/isoctrl/" + local_from + "/attach/";
                          destiny_attach_path =
                            "./app/storage/isoctrl/" + local_from + "/attach/";
                          origin_cl_path =
                            "./app/storage/isoctrl/" +
...
const unlock = (req, res) => {
  ...
  sql.query(
    "UPDATE misoctrls SET blocked = 0 WHERE isoid COLLATE utf8mb4_unicode_ci = ?",
    [isoid],
    (err, results) => {
      if (err) {
        res.status(401);
...
function downloadStatus3DPeriod() {
  sql.query(
    "SELECT tag, tpipes_id, `to`, `from`, claimed, issued FROM dpipes_view RIGHT JOIN misoctrls ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid ORDER BY misoctrls.id DESC",
    (err, results) => {
      let log = [];
      let lines = [];
      let ifc_ifd = "";
      ...
      for (let i = 0; i < results.length; i++) {
        if (lines.indexOf(results[i].tag) < 0) {
          log.push(
            "/" +
              results[i].tag +
              " STM ASS /TPI-EP-PROGRESS/PIPING/TOTAL-" +
              ifc_ifd
          );
          log.push("HANDLE ANY");
          log.push("ENDHANDLE");
          ...
          if (
            ...
            status == "LDE/Isocontrol" &&
            (results[i].issued == 0 || !results[i].issued)
          ) {
            status = "Issuer";
          ...
          if (status != "Recycle bin" && status != "On hold") {
            log.push(
              "/" +
                results[i].tag +
                " STM SET /TPI-EP-PROGRESS/PIPING/TOTAL-" +
                ifc_ifd +
                " /TL" +
                results[i].tpipes_id +
                "-" +
  ...
  await csv()
    ...
    .then((jsonObj) => {
      ...
      sql.query("SELECT isoid, tpipes_id FROM dpipes_view", (err, results) => {
        if (!results[0]) {
          ...
          for (let i = 0; i < isoids.length; i++) {
            sql.query(
              "UPDATE misoctrls set before_tpipes_id = ? WHERE isoid COLLATE utf8mb4_unicode_ci = ?",
      ...
      for (let i = 0; i < csv.length; i++) {
        if (csv[i].spo === "true") {
          sql.query(
            "UPDATE misoctrls LEFT JOIN dpipes_view ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid SET spo = 1 WHERE dpipes_view.tag = ?",
            [csv[i].tag],
            (err, results) => {
              if (err) {
                console.log("Error updating");
                ...
                sql.query(
                  ...
                  (err, results) => {
                    if (!results[0]) {
                      ...
                      sql.query(
                        "INSERT INTO dpipes(area_id, tag, diameter_id, calc_notes, tpipes_id, diameter, calc_notes_description, pid, stress_level, insulation, unit, fluid, seq, train, spec) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        [
                          areaid,
                          csv[i].tag,
                          diameterid,
                          ...
                          sql.query(
                            ...
                            (err, results) => {
                              if (!results[0]) {
                                ...
                                sql.query(
                                  "INSERT INTO pipectrls(tag, status_id) VALUES(?,?)",
              ...
              if (!results[0]) {
                ...
                sql.query(q, [level, tl], (err, results) => {
                  if (!results[0]) {
                    ...
                    sql.query(
                      "SELECT progress, max_tray FROM misoctrls WHERE filename = ?",
                      [lines[i].filename],
                      (err, results1) => {
                        if (!results1[0]) {
                          ...
                          const q2 =
                            "SELECT " +
                            type +
                            " as newp FROM ppipes WHERE level = ? AND tpipes_id = ?";
                          sql.query(
                            q2,
                            [max_tray, lines[i].before_tpipes_id],
                            (err, results) => {
                              if (!results[0]) {
                              } else {
                                const newProgress = results[0].newp;
                                sql.query(
                                  "UPDATE misoctrls SET progress = ?, realprogress = ? WHERE filename = ?",
                                  [
                                    newRealRrogress,
                                    newProgress,
                                    lines[i].filename,
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            ...
            sql.query(
              "SELECT id FROM tequis WHERE code = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  ...
                  sql.query(
                    "SELECT id FROM pequis WHERE percentage = ?",
                    [req.body[i][progress_index]],
                    (err, results) => {
                      if (!results[0]) {
                        res.json({ invalid: i }).status(401);
                        return;
                        ...
                        sql.query(
                          "INSERT INTO dequis(areas_id, tag, pequis_id, tequis_id, elements) VALUES (?,?,?,?,?)",
                          [
                            areaid,
                            req.body[i][tag_index],
                            percentageid,
                            typeid,
                            req.body[i][elements_index],
                          ],
                          (err, results) => {
                            if (err) {
                              console.log(err);
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tequis WHERE name = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "INSERT INTO eequis(areas_id, tequis_id, qty) VALUES (?,?,?)",
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tinsts WHERE code = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "SELECT id FROM pinsts WHERE percentage = ?",
                    [req.body[i][progress_index]],
                    (err, results) => {
                      if (!results[0]) {
                        res.json({ invalid: i }).status(401);
                        ...
                        sql.query(
                          "INSERT INTO dinsts(areas_id, tag, pinsts_id, tinsts_id) VALUES (?,?,?,?)",
                          [
                            areaid,
                            req.body[i][tag_index],
                            percentageid,
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tinsts WHERE name = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "INSERT INTO einsts(areas_id, tinsts_id, qty) VALUES (?,?,?)",
                    [areaid, typeid, req.body[i][qty_index]],
                    (err, results) => {
                      if (err) {
                        console.log(err);
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tcivils WHERE code = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "SELECT id FROM pcivils WHERE percentage = ?",
                    [req.body[i][progress_index]],
                    (err, results) => {
                      if (!results[0]) {
                        res.json({ invalid: i }).status(401);
                        ...
                        sql.query(
                          "INSERT INTO dcivils(areas_id, tag, pcivils_id, tcivils_id) VALUES (?,?,?,?)",
                          [
                            areaid,
                            req.body[i][tag_index],
                            percentageid,
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tcivils WHERE name = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "INSERT INTO ecivils(areas_id, tcivils_id, qty) VALUES (?,?,?)",
                    [areaid, typeid, req.body[i][qty_index]],
                    (err, results) => {
                      if (err) {
                        console.log(err);
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM telecs WHERE code = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "SELECT id FROM pelecs WHERE percentage = ?",
                    [req.body[i][progress_index]],
                    (err, results) => {
                      if (!results[0]) {
                        res
                          .status(401)
                          .send({ invalid: "Invaid percentage in some lines" });
                      } else {
                        const percentageid = results[0].id;

                        sql.query(
                          "INSERT INTO delecs(areas_id, tag, pelecs_id, telecs_id) VALUES (?,?,?,?)",
                          [
                            areaid,
                            req.body[i][tag_index],
                            percentageid,
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM telecs WHERE name = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "INSERT INTO eelecs(areas_id, telecs_id, qty) VALUES (?,?,?)",
                    [areaid, typeid, req.body[i][qty_index]],
                    (err, results) => {
                      if (err) {
                        console.log(err);
      ...
      if (
        ...
        sql.query(
          ...
          (err, results) => {
            const areaid = results[0].id;
            sql.query(
              "SELECT id FROM tpipes WHERE name = ?",
              [req.body[i][type_index]],
              (err, results) => {
                if (!results[0]) {
                  res.json({ invalid: i }).status(401);
                  ...
                  const typeid = results[0].id;
                  sql.query(
                    "INSERT INTO epipes(areas_id, tpipes_id, qty) VALUES (?,?,?)",
                    [areaid, typeid, req.body[i][qty_index]],
                    (err, results) => {
                      if (err) {
                        console.log(err);
...
const downloadInstrumentationModelled = (req, res) => {
  sql.query(
    "SELECT area, tag, type_inst, weight, status, progress FROM dinstsfull_view",
    (err, results) => {
      if (!results[0]) {
...
const downloadEquipmentModelled = (req, res) => {
  sql.query(
    "SELECT area, tag, type_equi, weight, status, progress FROM dequisfull_view",
    (err, results) => {
      if (!results[0]) {
        res.status(401).send("El historial esta vacio");
      } else {
...
const downloadCivilModelled = (req, res) => {
  sql.query(
    "SELECT area, tag, type_civil, weight, status, progress FROM dcivilsfull_view",
    (err, results) => {
      if (!results[0]) {
        res.status(401).send("El historial esta vacio");
      } else {
...
const downloadElectricalModelled = (req, res) => {
  sql.query(
    "SELECT area, tag, type_elec, weight, status, progress FROM delecsfull_view",
    (err, results) => {
      if (!results[0]) {
        res.status(401).send("El historial esta vacio");
      } else {
    ...
    sql.query("TRUNCATE bomtbl", (err, results) => {
      if (err) {
        ...
        for (let i = 9; i < rows.length; i++) {
          sql.query(
            "INSERT INTO bomtbl (unit, area, line, train, spec_code, weight) VALUES(?,?,?,?,?,?)",
            [
              rows[i][1],
              rows[i][2],
              rows[i][3],
...
const isocontrolWeights = async (req, res) => {
  ...
  sql.query(
    "SELECT SUM(total_weight) as modelledWeight FROM isocontrol_all_view WHERE area IS NOT null",
    (err, results) => {
      if (err) {
        res.status(401);
      } else {
        ...
        sql.query(
          "SELECT SUM(total_weight) as notModelledWeight FROM isocontrol_all_view WHERE area IS null",
          (err, results) => {
            if (err) {
              res.status(401);
            } else {
...
async function updateLines() {
  ...
  await csv()
    ...
    .then((jsonObj) => {
      ...
      for (let i = 0; i < csv.length; i++) {
        if (
          !(csv[i].tag + csv[i].unit + csv[i].fluid + csv[i].seq).includes(
            "unset"
          )
        ) {
          sql.query(
            "INSERT INTO `lines`(refno, tag, unit, fluid, seq, spec_code, pid, stress_level, calc_notes, insulation) VALUES(?,?,?,?,?,?,?,?,?,?)",
            [
              csv[i].refno,
              csv[i].tag,
              csv[i].unit,
    ...
    (err, results) => {
      if (err) {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          rows[i].iso_id =
            rows[i].unit + rows[i].area + rows[i].line + rows[i].train;
        }
        res.json(JSON.stringify(rows)).status(200);
      }
    }
...
const holds = async (req, res) => {
  sql.query(
    "SELECT dpipes_view.isoid, dpipes_view.tag as iso_tag, misoctrls.onhold, tpipes.code, misoctrls.revision, misoctrls.updated_at, misoctrls.`to`, misoctrls.user, misoctrls.role, holds.* FROM dpipes_view LEFT JOIN holds_isocontrol on dpipes_view.tag = holds_isocontrol.tag LEFT JOIN misoctrls ON dpipes_view.isoid COLLATE utf8mb4_unicode_ci = misoctrls.isoid LEFT JOIN tpipes ON dpipes_view.tpipes_id = tpipes.id LEFT JOIN holds ON dpipes_view.tag = holds.tag WHERE misoctrls.onhold = 1 GROUP BY misoctrls.isoid",
    (err, results) => {
      if (err) {
        res.status(401);
      } else {
            ...
            if (err) {
              ...
              for (let i = 0; i < data.length; i++) {
                ...
                if (data[i].tag && has_holds && has_holds != "") {
                  sql.query(
                    "INSERT INTO holds (tag, hold1, description1, hold2, description2, hold3, description3, hold4, description4, hold5, description5, hold6, description6, hold7, description7, hold8, description8, hold9, description9, hold10, description10) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    [
                      data[i].tag,
                      data[i].hold1,
                      data[i].description1,
                    ...
                    (err, results) => {
                      if (err) {
                        ...
                        if (has_holds) {
                          sql.query(
                            "UPDATE misoctrls JOIN dpipes_view ON dpipes_view.isoid COLLATE utf8mb4_unicode_ci = misoctrls.isoid SET misoctrls.onhold = 1 WHERE dpipes_view.tag = ? AND onhold != 2",
                            [data[i].tag],
                            (err, results) => {
                              if (err) {
                                console.log(err);
...
const uploadNotifications = (req, res) => {
  const n = req.body.n;
  sql.query(
    "SELECT DISTINCT model_id FROM model_has_roles WHERE role_id = 1 OR role_id = 2 OR role_id = 9",
    (err, results) => {
      if (!results[0]) {
        res.send({ success: 1 }).status(200);
      } else {
...
const lastUser = async (req, res) => {
  sql.query(
    "SELECT `user` FROM hisoctrls WHERE filename = ? ORDER BY id DESC LIMIT 1",
    [req.params.filename],
    (err, results) => {
      if (!results[0]) {
        res.status(401);
...
const exportLineIdGroup = async (req, res) => {
  sql.query(
    "SELECT * FROM isocontrol_lineid_group WHERE line_id is not null",
    (err, results) => {
      if (err) {
        res.status(401);
      } else {
...
const exportHolds = async (req, res) => {
  sql.query(
    "SELECT tag, hold1, description1, hold2, description2, hold3, description3, hold4, description4, hold5, description5, hold6, description6, hold7, description7, hold8, description8, hold9, description9, hold10, description10 FROM holds",
    (err, results) => {
      if (err) {
        res.status(401);
      } else {
...
const exportHoldsNoProgress = async (req, res) => {
  sql.query(
    "SELECT isoid, revision, updated_at, `from`, user, comments FROM misoctrls WHERE misoctrls.to = ?",
    ["On hold"],
    (err, results) => {
      for (let i = 0; i < results.length; i++) {
        results[i].updated_at =
          results[i].updated_at.toString().substring(0, 10) +
          " " +
          results[i].updated_at.toString().substring(11, 24);
      }
      res.json(JSON.stringify(results)).status(200);
    }
  );
...
const revision = async (req, res) => {
  sql.query(
    "SELECT revision, issuer_date, issuer_designation, issuer_draw, issuer_check, issuer_appr FROM misoctrls WHERE filename = ?",
    [req.params.fileName],
    (err, results) => {
      if (!results[0]) {
        res.status(401);
...
const submitRevision = async (req, res) => {
  ...
  sql.query(
    "UPDATE misoctrls SET issuer_date = ?, issuer_designation = ?, issuer_draw = ?, issuer_check = ?, issuer_appr = ? WHERE filename = ?",
...
function downloadIssuedTo3D() {
  let exists = false;
  sql.query(
    "SELECT dpipes_view.tag, revision, issued, issuer_date, issuer_designation, issuer_draw, issuer_check, issuer_appr FROM dpipes_view JOIN misoctrls ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid WHERE `to` = ?",
    ["Issuer"],
    (err, results) => {
      if (err) {
        console.log(err);
      ...
      if (!results[0]) {
        ...
        for (let i = 0; i < results.length; i++) {
          if (
            results[i].issuer_date &&
            results[i].issuer_designation &&
            results[i].issuer_draw &&
            results[i].issuer_check &&
            results[i].issuer_appr
          ) {
            exists = true;
            ...
            log.push(
              ":TP-REV-DESIGNATION '" + results[i].issuer_designation + "'"
            );
...
const excludeHold = async (req, res) => {
  fileName = req.params.fileName;
  await sql.query(
    "UPDATE misoctrls SET onhold = 2, `to` = `from` WHERE filename = ?",
    [fileName],
    async (err, results) => {
      if (err) {
        console.log(err);
        ...
        await sql.query(
          "UPDATE misoctrls SET `from` = ? WHERE filename = ?",
          ["On hold", fileName],
          (err, results) => {
            if (err) {
              console.log(err);
...
const sendHold = (req, res) => {
  const fileName = req.body.fileName;
  sql.query(
    "UPDATE misoctrls SET onhold = 1, `from` = `to` WHERE filename = ?",
    [fileName],
    (err, results) => {
      if (err) {
        console.log(err);
        ...
        sql.query(
          "UPDATE misoctrls SET `to` = ? WHERE filename = ?",
          ["On hold", fileName],
          (err, results) => {
            if (err) {
              console.log(err);
...
const getFilenamesByUser = (req, res) => {
  ...
  sql.query(
    "SELECT misoctrls.isoid from misoctrls LEFT JOIN users ON misoctrls.user = users.name WHERE users.email = ? AND misoctrls.role = ?",
    [email, role],
    (err, results) => {
...
const createByPass = (req, res) => {
  ...
  sql.query("SELECT id FROM users WHERE email = ?", [email], (err, results) => {
    if (!results[0]) {
      ...
      sql.query(
        "SELECT id FROM bypass ORDER BY id DESC LIMIT 1",
        (err, results) => {
          let tag = "BP000001";
          if (results[0]) {
            tag =
          ...
          sql.query(
            ...
            (err, results) => {
              if (err) {
                ...
                sql.query(
                  "SELECT name FROM tbypass WHERE id = ?",
                  [type],
                  (err, results) => {
                    const t = results[0].name;
                    sql.query(
                      "SELECT isoid FROM misoctrls WHERE id = ?",
                      [iso_id],
                      (err, results) => {
                        if (!results[0]) {
                          res.status(401);
                          ...
                          const html_message =
                            ...
                            "</p>";
                          sql.query(
                            "SELECT email FROM users JOIN model_has_roles ON users.id = model_has_roles.model_id JOIN roles ON model_has_roles.role_id = roles.id WHERE roles.id = 15 GROUP BY email",
                            (err, results) => {
                              if (!results[0]) {
                              } else {
                                for (let i = 0; i < results.length; i++) {
...
const getByPassData = async (req, res) => {
  sql.query(
    "SELECT bypass.id, bypass.comments, misoctrls.isoid, tbypass.name as type, bypass.tag, bypass.note, users.name as user, users.email, bstatus.name as status, bypass.updated_at as date FROM bypass LEFT JOIN misoctrls ON bypass.misoctrls_id = misoctrls.id LEFT JOIN tbypass ON bypass.tbypass_id = tbypass.id LEFT JOIN users ON bypass.user_id = users.id LEFT JOIN bstatus on bypass.bstatus_id = bstatus.id",
    (err, results) => {
      if (!results[0]) {
  ...
  sql.query(
    ...
    (err, results) => {
      if (err) {
        ...
        sql.query(
          "SELECT tag, users.email FROM bypass LEFT JOIN users ON bypass.user_id = users.id WHERE bypass.id = ?",
          [id],
          (err, results) => {
            ...
            transporter.sendMail(
              {
                ...
                to: email,
                subject:
                  "ByPass " + tag + " has been accepted. " + answer + ".",
                text: tag,
...
const rejectByPass = async (req, res) => {
  ...
  const comments = req.body.comments;
  sql.query(
    "UPDATE bypass SET bstatus_id = 4, comments = ? WHERE id = ?",
    [comments, id],
    (err, results) => {
      if (err) {
        res.status(401);
...
const naByPass = async (req, res) => {
  ...
  const comments = req.body.comments;
  sql.query(
    "UPDATE bypass SET bstatus_id = 5, comments = ? WHERE id = ?",
    [comments, id],
    (err, results) => {
      if (err) {
        res.status(401);
...
const editByPass = async (req, res) => {
  ...
  sql.query(
    "UPDATE bypass SET tbypass_id = ?, note = ? WHERE id = ?",
    [type, notes, iso_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const closeByPass = async (req, res) => {
  const iso_id = req.body.id;
  sql.query(
    "SELECT bstatus_id FROM bypass WHERE id = ?",
    [iso_id],
    (err, results) => {
      if (!results[0]) {
        res.status(401);
...
const acceptByPass = async (req, res) => {
  const iso_id = req.body.id;
  sql.query(
    "UPDATE bypass SET bstatus_id = 8 WHERE id = ?",
    [iso_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const exportByPass = async (req, res) => {
  sql.query(
    "SELECT bypass.tag, misoctrls.isoid, tbypass.name as type, bypass.updated_at as date, users.name as user, bypass.note, bypass.comments, bstatus.name as status FROM bypass LEFT JOIN misoctrls ON bypass.misoctrls_id = misoctrls.id LEFT JOIN tbypass ON bypass.tbypass_id = tbypass.id LEFT JOIN users ON bypass.user_id = users.id LEFT JOIN bstatus on bypass.bstatus_id = bstatus.id",
    (err, results) => {
      if (!results[0]) {
        res.status(401);
      } else {
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        const revision = results[0].revision;
        sql.query(
          "SELECT * FROM misoctrls WHERE isoid = ? AND revision > ?",
          [isoid, revision],
          (err, results) => {
            if (results[0]) {
              ...
              sql.query(
                "SELECT `to` FROM misoctrls WHERE isoid = ? AND (issued = 0 OR issued IS NULL)",
                [isoid, revision],
                (err, results) => {
                  if (!results[0]) {
                    res.send({ cancellable: false });
                      ...
                      sql.query(
                        ...
                        (err, results) => {
                          if (!results[0]) {
                            ...
                            const isoid = results[0].isoid;
                            sql.query(
                              "SELECT filename FROM misoctrls WHERE isoid = ?",
                              [isoid],
                              (err, results) => {
                                const newRevFilename = results[0].filename;
                                sql.query(
                                  "DELETE FROM misoctrls WHERE isoid = ? AND (issued = 0 OR issued IS NULL) AND `to` = ?",
                                  [isoid, "Design"],
                                  (err, results) => {
                                    if (err) {
                                      console.log(err);
                                      ...
                                      sql.query(
                                        ...
                                        (err, results) => {
                                          ...
                                          sql.query(
                                            "INSERT INTO hisoctrls (filename, isoid, user, role) VALUES (?,?,?,?)",
                                            [
                                              filename,
                                              isoid,
                                              username,
                                      ...
                                      sql.query(
                                        "UPDATE misoctrls SET requested = 0 WHERE filename = ?",
                                        [filename],
                                        (err, results) => {
                                          if (err) {
                                            console.log(err);
...
const issuedFiles = async (req, res) => {
  if (process.env.NODE_PROGRESS === "1") {
    sql.query(
      'SELECT misoctrls.*, dpipes_view.*, tpipes.`name`, tpipes.weight, tpipes.`code` FROM misoctrls LEFT JOIN dpipes_view ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid LEFT JOIN tpipes ON dpipes_view.tpipes_id = tpipes.id WHERE misoctrls.to = "LDE/IsoControl" && issued = 1 ORDER BY issued_date DESC',
      (err, results) => {
        res.json({
          rows: results,
        });
    ...
    sql.query(
      'SELECT * FROM misoctrls WHERE misoctrls.to = "LDE/IsoControl" && issued = 1',
      (err, results) => {
        res.json({
          rows: results,
        });
...
const getDesigners = async (req, res) => {
  //Get de los usuarios con rol de diseño
  sql.query(
  ...
  sql.query("SELECT * FROM estimated_pipes_view", (err, results) => {
    ...
    if (err) {
      ...
      for (let i = 0; i < results.length; i++) {
        //Por cada linea
        if (!results[i].type) {
          //Si no tiene tipo (por lo tanto es TL3)
          if (process.env.NODE_MMDN == "0") {
            if (results[i].diameter < 2.0) {
...
const submitModelledEstimatedPipes = async (req, res) => {
  //Guardamos las lineas del likeexcel de isocontrol (main)
  ...
  for (let i = 0; i < new_pipes.length; i++) {
    ...
    if (new_pipes[i]["Line reference"] == "deleted") {
      //Si se a eliminado el tag se elimina la linea de la bd
      sql.query(
      ...
      sql.query(
        "SELECT id FROM `lines` WHERE tag = ?",
...
const submitFeedPipes = async (req, res) => {
  ...
  for (let i = 0; i < new_pipes.length; i++) {
    ...
    if (new_pipes[i]["Line reference"] == "deleted") {
      //Si no tiene line ref se elimina
      sql.query(
                  ...
                  await sql.query(
                    ...
                    async (err, results) => {
                      ...
                      if (new_pipes[i].id) {
                        ...
                        sql.query(
                          "UPDATE feed_pipes SET line_refno = ?, area_id = ?, diameter = ?, train = ?, status=?, owner_id = ? WHERE id = ?",
                          [
                            line_refno,
                            area_id,
                            new_pipes[i].Diameter,
                        ...
                        sql.query(
                          "INSERT INTO feed_pipes(line_refno, area_id, diameter, train, status, owner_id) VALUES(?,?,?,?,?,?)",
                          [
                            line_refno,
                            area_id,
                            new_pipes[i].Diameter,
                            ...
                            if (err) {
                              ...
                              if (new_pipes[i].Status == "MODELLED(100%)") {
                                ...
                                sql.query(
                                  ...
                                  [line_refno],
                                  (err, results) => {
                                    //Cogemos el ide de la linea del feed
                                    if (!results[0]) {
                                      res.status(401);
                                    } else {
                                      const feed_id = results[0].id;
                                      sql.query(
                                        ...
                                        [feed_id],
                                        (err, results) => {
                                          //Cogemos el id de la linea de la tabla de estimadas
                                          if (!results[0]) {
                                            //Si no existe creamos la linea en las estimadas
                                            sql.query(
                                              "INSERT INTO estimated_pipes(line_ref_id, tag, feed_id, unit, area_id, fluid, sequential, spec, diameter, insulation, train) VALUES(?,?,?,?,?,?,?,?,?,?,?)",
                                              [
                                                line_ref_id,
...
const submitModelledEstimatedCustomPipes = async (req, res) => {
  //Lo mismo que submitModelledEstimatedPipes pero el estado de a linea puede ser forzado (F)
  const new_pipes = req.body.rows;
            ...
            sql.query(
              ...
              (err, results) => {
                if (!results[0]) {
                  ...
                  if (new_pipes[i].id) {
                    sql.query(
                      "UPDATE estimated_pipes SET line_refno = ?, area_id = ?, diameter = ?, train = ? WHERE id = ?",
                      [
                        line_refno,
                        area_id,
                        new_pipes[i].Diameter,
                    ...
                    sql.query(
                      "INSERT INTO estimated_pipes(line_refno, area_id, diameter, train) VALUES(?,?,?,?)",
                      [
                        line_refno,
                        area_id,
                        new_pipes[i].Diameter,
                  ...
                  if (new_pipes[i]["Status"] == "MODELLED(F)") {
                    sql.query(
                      "SELECT * FROM pipectrls WHERE tag = ?",
                      new_pipes[i]["Tag"],
                      (err, results) => {
                        if (!results[0]) {
                          let initial_state = 0;
                    ...
                    sql.query(
                      "SELECT * FROM custom_status_pipes WHERE tag = ?",
                      new_pipes[i]["Tag"],
                      (err, results) => {
                        if (!results[0]) {
                          sql.query(
                            "INSERT INTO custom_status_pipes(tag, custom_status) VALUES(?,?)",
                            [new_pipes[i]["Tag"], 0],
                            (err, results) => {
                              if (err) {
                                console.log(err);
                          ...
                          sql.query(
                            "UPDATE custom_status_pipes SET custom_status = ? WHERE tag = ?",
                            [0, new_pipes[i]["Tag"]],
                            (err, results) => {
                              if (err) {
                                console.log(err);
                    ...
                    sql.query(
                      ...
                      (err, results) => {
                        if (results[0]) {
                          sql.query(
                            "DELETE FROM pipectrls WHERE tag = ?",
                            [new_pipes[i]["Tag"]],
                            (err, results) => {
                              if (err) {
                                console.log(err);
    ...
    await sql.query(
      ...
      async (err, results) => {
        if (!results) {
          ...
          const user_id = results[0].id;
          await sql.query(
            "SELECT id FROM owners WHERE line_refno = ?",
            owners[i][1],
            async (err, results) => {
...
const modelledEstimatedHolds = async (req, res) => {
  //Get de los holds de las lineas modeladas y estimadas
  sql.query(
...
const getAllHolds = async (req, res) => {
  //Get de todos los holds de las lineas del proyecto
  const tag = req.params.tag;
  let holds3d = [];
  let holdsIso = [];
  sql.query(
    "SELECT holds.* FROM holds WHERE holds.tag = ?",
    [tag],
    (err, results) => {
      if (results[0]) {
        holds3d = results;
      ...
      sql.query(
        "SELECT holds_isocontrol.id, holds_isocontrol.description FROM holds_isocontrol WHERE holds_isocontrol.tag = ?",
        [tag],
        (err, results) => {
          if (results[0]) {
            for (let i = 0; i < results.length; i++) {
...
const submitHoldsIso = async (req, res) => {
  //Crear un hold para una iso
  const holds = req.body.rows;
  const tag = req.body.tag;
  for (let i = 0; i < holds.length; i++) {
    if (!holds[i]["description"]) {
      //Si se ha eliminado de la tabla lo eliminamos de la bd
      sql.query(
        "DELETE FROM holds_isocontrol WHERE id = ?",
        [holds[i].id],
        (err, results) => {
          if (err) {
            console.log(err);
      ...
      //Si el hold no tiene id es nuevo, lo instertamos
      sql.query(
        "INSERT INTO holds_isocontrol(tag, description) VALUES(?,?)",
      ...
      //Si tiene id ya existia, lo actualizamos
      sql.query(
        "UPDATE holds_isocontrol SET description = ? WHERE id = ?",
...
const getIsocontrolHolds = async (req, res) => {
  ...
  const tag = req.params.tag;
  sql.query(
    "SELECT description FROM holds_isocontrol WHERE tag = ?",
    [tag],
    (err, results) => {
      if (!results[0]) {
...
const getEstimatedMatWeek = async (req, res) => {
  //Get de las estimadas en funcion del material y la semana
  //Cogemos cada valor estimado por semana y material
  sql.query(
    "SELECT week, estimated, material_id, name FROM epipes_new LEFT JOIN materials ON material_id = materials.id ORDER BY material_id, week",
    (err, results) => {
      if (!results[0]) {
        ...
        sql.query("SELECT starting_date FROM project_span", (err, results) => {
          //Cogemos el inicio y fin del proyecto
          ...
          ); //Cantidad de dias entre el inicio del año y el dia de inicio del proyecto
          const initial_week = Math.ceil(
            (start.getDay() + 1 + numberOfDays) / 7
          ); //Dividimos esa cantidad entre 7 para obtener a que semana del año pertenece el primer dia del proyecto
          let counter = 0;
          let mat = estimated[0].material_id;
          for (let i = 0; i < estimated.length; i++) {
            //Por cada estimacion
            if (mat == estimated[i].material_id) {
              //Si pertenece al mismo material que la estimacion anterior
...
const getForecastMatWeek = async (req, res) => {
  //Get del forecast de cada material
  sql.query(
    "SELECT week, estimated, material_id, name FROM forecast LEFT JOIN materials ON material_id = materials.id ORDER BY material_id, week",
    (err, results) => {
      if (!results[0]) {
...
const getMaterialsPipingClass = async (req, res) => {
  //Get de la piping class de cada material
  sql.query(
    "SELECT materials.id as material_id, materials.name as material, pipingclass.id as piping_id, pipingclass.name as piping FROM pipingclass LEFT JOIN materials ON materials.id = pipingclass.material_id",
    (err, results) => {
      if (!results[0]) {
...
const getProjectSpan = async (req, res) => {
  //Get del inicio y fin del proyecto
  sql.query(
    "SELECT starting_date, finishing_date FROM project_span",
    (err, results) => {
      if (!results[0]) {
...
const submitProjectSpan = async (req, res) => {
  //Submit del inicio y fin del proyecto
  //Formateamos los inputs
  const start =
    req.body.span["Starting date"].substring(6, 10) +
    "-" +
    req.body.span["Starting date"].substring(3, 5) +
    "-" +
    req.body.span["Starting date"].substring(0, 2);
  const finish =
    req.body.span["Finishing date"].substring(6, 10) +
    "-" +
    req.body.span["Finishing date"].substring(3, 5) +
    "-" +
    req.body.span["Finishing date"].substring(0, 2);
  sql.query(
    "UPDATE project_span SET starting_date = ?, finishing_date = ?",
    [start, finish],
    (err, results) => {
      ...
      if (err) {
        ...
        const weeks =
          (new Date(finish) - new Date(start)) / (1000 * 60 * 60 * 24) / 7;
        let currentWeeks = 0;
        sql.query(
          "SELECT DISTINCT week FROM epipes_new ORDER BY week DESC LIMIT 1",
          (err, results) => {
            //Cogemos la cantidad de semanas que habia en el proyecto antes de actualizar el span
            if (results[0]) {
              currentWeeks = results[0].week;
            }
            if (weeks < currentWeeks) {
              //Si antes del cambio el proyecto tenia mas semanas
              sql.query(
                "DELETE FROM epipes_new WHERE week > ?",
                [weeks],
                (err, results) => {
              ...
              sql.query(
                "DELETE FROM forecast WHERE week > ?",
                [weeks],
                (err, results) => {
              ...
              sql.query(
                "DELETE FROM epipes_ifd WHERE week > ?",
                [weeks],
                (err, results) => {
              ...
              sql.query(
                "DELETE FROM forecast_ifd WHERE week > ?",
                [weeks],
                (err, results) => {
              ...
              //Si antes del cambio el proyecto tenia menos semanas
              sql.query("SELECT id FROM materials", (err, results) => {
                //Cogemos los materiales del proyecto
                if (!results[0]) {
                  res.send({ success: false }).status(401);
                } else {
                  let materials_ids = [];
                  ...
                  for (let i = 0; i < newWeeks.length; i++) {
                    //Por cada nueva semana
                    for (let j = 0; j < materials_ids.length; j++) {
                      //Por cada material
                      sql.query(
                        "INSERT INTO epipes_new(week, material_id) VALUES(?,?)",
                        [newWeeks[i], materials_ids[j]],
                        (err, results) => {
                      ...
                      sql.query(
                        "INSERT INTO forecast(week, material_id) VALUES(?,?)",
                        [newWeeks[i], materials_ids[j]],
                        (err, results) => {
                    ...
                    sql.query(
                      "INSERT INTO epipes_ifd(week) VALUES(?)",
                      [newWeeks[i]],
                      (err, results) => {
                    ...
                    sql.query(
                      "INSERT INTO forecast_ifd(week) VALUES(?)",
                      [newWeeks[i]],
                      (err, results) => {
                        //Insert de las nuevas semanas en estimadas para ifd
                        if (err) {
            ...
            sql.query(
              "SELECT DISTINCT week FROM eweights ORDER BY week DESC LIMIT 1",
              (err, results) => {
                //Cogemos la cantidad de semanas del proyecto
                let currentWeeks = 0;
                const weeks =
                ...
                if (weeks < currentWeeks) {
                  //Si antes del cambio el proyecto tenia mas semanas
                  sql.query(
                    "DELETE FROM eweights WHERE week > ?",
                    [weeks],
                    (err, results) => {
                  ...
                  for (let i = 0; i < newWeeks.length; i++) {
                    sql.query(
                      "INSERT INTO eweights(week) VALUES(?)",
                      [newWeeks[i]],
                      (err, results) => {
...
const submitPipingClass = async (req, res) => {
  //Submit de las piping class de los materiales
  const pipingClass = req.body.piping;
  for (let i = 0; i < pipingClass.length; i++) {
    //Por cada piping class
    if (
      !pipingClass[i]["PipingClass"] ||
      pipingClass[i]["PipingClass"] == "" ||
      !pipingClass[i]["Material"] ||
      pipingClass[i]["Material"] == ""
    ) {
      //Eliminamos la piping class
      sql.query(
        "DELETE FROM pipingclass WHERE id = ?",
        [pipingClass[i]["id"]],
        (err, results) => {
          if (err) {
            console.log(err);
      ...
      sql.query(
        ...
        [pipingClass[i]["Material"]],
        (err, results) => {
          //Cogemos el id del material
          if (!results[0]) {
            res.send({ success: false }).status(401);
          } else {
            const material_id = results[0].id;
            sql.query(
              "SELECT * FROM pipingclass WHERE id = ?",
              [pipingClass[i]["id"]],
              (err, results) => {
                //Cogemos la id de la pipingclass
                if (!results[0]) {
                  //Si no existe quiere decir que la pipingclass es nueva, la insertamos
                  sql.query(
                    "INSERT INTO pipingclass(name, material_id) VALUES(?,?)",
                  ...
                  sql.query(
                    "UPDATE pipingclass SET name = ?, material_id = ? WHERE id = ?",
...
const submitMaterials = async (req, res) => {
  //Submit de materiales
  const materials = req.body.materials;
  for (let i = 0; i < materials.length; i++) {
    if (!materials[i]["Material"] || materials[i]["Material"] == "") {
      //Eliminamos el material si se elimina de la tabla
      await sql.query(
      ...
      await sql.query(
        ...
        async (err, results) => {
          //Cogemos el id del material
          if (!results[0]) {
            //Si no existe es que el material es nuevo
            await sql.query(
              ...
              async (err, results) => {
                ...
                */
                      await sql.query(
                        "SELECT * FROM project_span",
                        async (err, results) => {
                          //Cogemos el span del proyecto
                          if (results[0]) {
                            let week = Math.floor(
                              (new Date(results[0].finishing_date) -
                                new Date(results[0].starting_date)) /
                                (1000 * 60 * 60 * 24) /
                                7
                            );

                            for (let w = 1; w < week + 1; w++) {
                              //Por cada semana del proyecto
                              await sql.query(
                                "INSERT INTO epipes_new(week, material_id) VALUES(?,?)",
                                [w, material_id],
                                (err, results) => {
            ...
            //Si ya existe el id actualizamos el material
            await sql.query(
...
const submitEstimatedForecast = async (req, res) => {
  //Submit de la estimacion y el forecast
  const material_id = req.body.material_id;
  const estimated = req.body.estimated;
  const forecast = req.body.forecast;

  Object.keys(estimated).map(function (key, index) {
    //Para cada valor de estimacion
    sql.query(
      "UPDATE epipes_new SET estimated = ? WHERE material_id = ? AND week = ?",
      [estimated[key], material_id, key],
      (err, results) => {
  ...
  Object.keys(forecast).map(function (key, index) {
    sql.query(
      "UPDATE forecast SET estimated = ? WHERE material_id = ? AND week = ?",
      [forecast[key], material_id, key],
      (err, results) => {
...
const getEstimatedByMaterial = async (req, res) => {
  //Get de la estimacion de cada material
...
const getIssuedByMatWeek = async (req, res) => {
  //Get de las isometricas emitidas divididas por material
  sql.query("SELECT * FROM issued_material_view", (err, results) => {
    //Cogemos las emitidas
    if (!results[0]) {
      ...
      sql.query("SELECT starting_date FROM project_span", (err, results) => {
        ...
        if (!results[0]) {
          ...
          for (let i = 0; i < issued_isos.length; i++) {
            //Por cada emitida
            let week = Math.floor(
              (new Date(issued_isos[i].issuer_date) - new Date(start)) /
                (1000 * 60 * 60 * 24) /
                7
            ); //Semana en la que se emitio la iso
            if (material_id == issued_isos[i].material_id) {
              //Si el material es el mismo que en el valor de estimacion anterior
              if (issued_mat[week]) {
                //Si esa semana ya tenia alguna emitida sumamos 1
                issued_mat[week] += 1;
              } else {
...
const getIssuedWeightByMatWeek = async (req, res) => {
  //Lo mismo que getIssuedByMatWeek pero en vez de por unidades por peso
    ...
    if (!results[0]) {
      ...
      sql.query("SELECT starting_date FROM project_span", (err, results) => {
        if (!results[0]) {
          ...
          for (let i = 0; i < issued_isos.length; i++) {
            let week = Math.floor(
              (new Date(issued_isos[i].issued_date) - new Date(start)) /
                (1000 * 60 * 60 * 24) /
                7
            );
            if (material_id == issued_isos[i].material_id) {
...
const getEstimatedForecastWeight = async (req, res) => {
  //Get del peso de las estimadas y modeladas
...
const submitEstimatedForecastWeight = async (req, res) => {
  ...
  Object.keys(estimated).map(function (key, index) {
    sql.query(
      "UPDATE eweights SET estimated = ?, forecast = ? WHERE week = ?",
          ...
          (err, results) => {
            ...
            if (!results[0]) {
              ...
              for (let i = 0; i < assignations.length; i++) {
                //Para cada owner
                let week = Math.floor(
                  (new Date(assignations[i].assignation_date) -
                    new Date(start)) /
                    (1000 * 60 * 60 * 24) /
                    7
                ); //Obtenemos la semana en la que al iso fue asignada al owner
              ...
              sql.query(
                "SELECT * FROM design_transactions_view",
                (err, results) => {
                  //Cogemos todas las transacciones que se han hecho sobre cada iso en las que ha intervenido diseño
                  if (!results[0]) {
                    //Si no hay transacciones en diseño solo se pueden haber asignado, pero el usuario no ha hecho nada con ellas aun
                    let total_weeks = Math.floor(
                      (new Date() - new Date(start)) / (1000 * 60 * 60 * 24) / 7
                    );
                    Object.keys(user_isos).map(function (user, index) {
                      ...
                      for (let w = 1; w < total_weeks + 1; w++) {
                        ...
                        let remaining = 0;
                        if (w > 1) {
                          //Excepto en la semana incicial, el valor de isos restantes se inicializa con el valor de la semana anterior
                          remaining = user_isos[user]["remaining"][w - 1];
                        }
                        if (user_isos[user]["assigned"]) {
                          //Si se le han asignado isos esta semana al usuario
                    ...
                    //Si existen transacciones ha habido movimiento con las isos
                    ...
                    for (let i = 0; i < transactions.length; i++) {
                      ...
                      let week = Math.floor(
                        (new Date(transactions[i].created_at) -
                          ...
                          (1000 * 60 * 60 * 24) /
                          7
                      ); //Obtenemos la semana en la que se hizo la transaccion
                      if (current_user === transactions[i].name) {
                        //Si la transaccion la ha hecho el usuario que estamos controlando
                        if (
                          transactions[i].from === "Design" &&
                          transactions[i].to !== "Design"
                        ) {
                          //Si la iso ha ido de diseño a otro sitio se suma a las enviadas
                          if (user_sent[week]) {
                            user_sent[week] += 1;
                          } else {
                            user_sent[week] = 1;
                          ...
                          //Si se ha cancelado la verificacion se cuenta como retornada
                          ...
                          //Si la iso viene de issued es una nueva revision, cuenta como iso nueva asignada asignada
                          ...
                          //Si la iso vuevle a diseño, cuenta como retornada
                          if (user_returned[week]) {
                            user_returned[week] += 1;
                          } else {
                            user_returned[week] = 1;
                        ...
                        //Si la transaccion es de otro usuario
                    ...
                    Object.keys(user_isos).map(function (key, index) {
                      //Por cada semana de este usuario se calculan las restantes
                    ...
                    for (let i = 0; i < transactions.length; i++) {
                      ...
                      if (current_user === transactions[i].name) {
                        if (
                          ...
                          if (user_isos[current_user]["assigned"][week]) {
                            user_isos[current_user]["assigned"][week] +=
...
const getIsosByUserWeek = async (req, res) => {
  //Lo mismo que getIsosByUserWeekDesign pero para el resto de roles. La difenencia esta en que las isos asignadas van en funcion del claim de las isos en vez de por el owner
  ...
  sql.query(
    ...
    (err, results) => {
      if (!results[0]) {
        ...
        sql.query(
          "SELECT `users`.`name` as `user`, `roles`.`name` as `role` FROM users JOIN model_has_roles ON users.id = model_has_roles.model_id LEFT JOIN roles ON model_has_roles.role_id = roles.id WHERE roles.id >= 2 AND roles.id <= 8 ORDER BY `roles`.`name`",
          (err, results) => {
            if (!results[0]) {
              res.send({ user_isos: [] }).status(200);
            } else {
              ...
              for (let i = 0; i < roles.length; i++) {
                //Por cada rol que tiene el usuario

                if (role == results[i].role) {
                  //Si ya existia algun usuario con ese rol, añadimos el nuevo usuario al diccionario
                  ...
                  //Si no existia ningun usuario con ese rol, creamos el diccionario para el rol y añadimos el nuevo usuario al diccionario
                  role = results[i].role;
                  user_isos[role] = {};
                  user_isos[role][roles[i].user] = {};
                  user_isos[role][roles[i].user]["claimed"] = {};
              ...
              sql.query("SELECT * FROM transactions_view", (err, results) => {
                //A partir de aqui es lo mismo que en getIsosByUserWeekDesign con la diferencia del claim
                if (!results[0]) {
                  ...
                  for (let i = 0; i < transactions.length; i++) {
                    let w = Math.floor(
                      (new Date(transactions[i].created_at) - new Date(start)) /
                        (1000 * 60 * 60 * 24) /
                        7
                    );
                    if (transactions[i].to == "Claimed") {
                      if (
                        !owners_by_role[
                          transactions[i].filename +
                            transactions[i].role +
                            transactions[i].revision
                        ]
                      ) {
                        if (
                          user_isos[transactions[i].role][transactions[i].name][
                            "claimed"
                          ][w]
                        ) {
                          user_isos[transactions[i].role][transactions[i].name][
                      ...
                      if (comments[0] == "FU") {
                        if (
                          ...
                          if (
                            user_isos[comments[2]][comments[1]]["returned"][w]
                          ) {
                            user_isos[comments[2]][comments[1]]["returned"][
                              w
                            ] -= 1;
                          } else {
                            user_isos[comments[2]][comments[1]]["returned"][w] =
                          ...
                          if (
                            user_isos[comments[2]][comments[1]]["claimed"][w]
                          ) {
                            user_isos[comments[2]][comments[1]]["claimed"][
                              w
                            ] -= 1;
                          } else {
                            user_isos[comments[2]][comments[1]]["claimed"][w] =
                        ...
                        if (
                          ...
                          (order.indexOf(transactions[i].to) <=
                            order.indexOf(transactions[i].from) ||
                            transactions[i].to == "Cancel verify") &&
                          transactions[i].to != "Design" &&
                          transactions[i].to != "Verify"
                        ) {
                          if (
                            user_isos[transactions[i].to][
                              owners_by_role[
                                transactions[i].filename +
                                  transactions[i].to +
                                  transactions[i].revision
                        ...
                        if (
                          owners_by_role[
                            transactions[i].filename +
                              transactions[i].role +
                              transactions[i].revision
                          ] == transactions[i].comments &&
                          transactions[i].comments != null &&
                          transactions[i].role == "StressLead"
                        ) {
                          if (
                            user_isos["SupportsLead"][transactions[i].comments][
                              "claimed"
                            ][w]
                          ) {
                            user_isos["SupportsLead"][transactions[i].comments][
                      ...
                      if (
                        transactions[i].role == "SupportsLead" &&
                        transactions[i].verify == 1
                      ) {
                        if (
                          owners_by_role[
                            transactions[i].filename +
...
const getWeightByUserWeek = async (req, res) => {
  //Lo mismo que getIsosByUserWeek pero por peso en vez de por unidad
  sql.query(
    "SELECT starting_date, finishing_date FROM project_span",
    (err, results) => {
      if (!results[0]) {
                      ...
                      if (comments[0] == "FU") {
                        ...
                        if (
                          ...
                          if (
                            ...
                            ]["returned"][w] += transactions[i].weight;
                          } else {
                            user_isos[transactions[i].to][
                              owners_by_role[
                                transactions[i].filename +
...
const trayCount = async (req, res) => {
  //Contador de cuantas isos hay sin reclamar en cada bandeja
    ...
    if (!results[0]) {
      ...
      for (let i = 0; i < results.length; i++) {
        if (results[i].status == "MODELLING(50%)") {
          //Si el status es del MODELLED 50% sumamos 50 al progreso
          progress += 50;
        } else if (results[i].status == "TOCHECK(80%)") {
          //Si el status es del TO CHECK 50% sumamos 80 al progreso
          progress += 80;
        } else if (results[i].status == "MODELLED(100%)") {
          //Si es de MODELLED 100% sumamos 100
          progress += 100;
        }
      }
...
cron.schedule("0 1 * * *", async () => {
  //Cada dia a la 1AM se guarda el progreso que hay en ese momento
  saveFeedWeight();
});
...
const getFeedForecast = async (req, res) => {
  ...
  sql.query("SELECT * FROM feed_forecast", (err, results) => {
    if (!results[0]) {
...
const submitFeedForecast = async (req, res) => {
  // Se guarda el progreso del feed
  ...
  Object.keys(estimated).map(function (key, idx) {
    ...
    sql.query(
      ...
      [dayNum],
      (err, results) => {
        //Cogemos el forecast de ese dia
        if (!results[0]) {
          ...
          sql.query(
            "UPDATE feed_forecast SET estimated = ?, forecast = ? WHERE day = ?",
  ...
  sql.query("SELECT * FROM feed_pipes_view", (err, results) => {
    if (!results[0]) {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        let tag =
          results[i][tag_order[0]] +
          "-" +
          results[i][tag_order[1]] +
          "-" +
          results[i][tag_order[2]] +
          "-" +
          results[i][tag_order[3]] +
          "-" +
          results[i][tag_order[4]] +
          "-" +
          results[i][tag_order[5]] +
          "-" +
          results[i][tag_order[6]] +
          "_" +
          results[i][tag_order[7]];
        log.push("/Cpipes");
        let pipe =
          "New :Cpipes /" +
          tag +
          " :DiametersRefFromCpipes /" +
          results[i].line_reference +
          "_" +
          results[i].diameter +
          " :720001MTOAREA '" +
          results[i].area +
...
function downloadStatusIFD() {
  sql.query("SELECT * FROM estimated_pipes_view", (err, results) => {
    if (!results[0]) {
      ...
      for (let i = 0; i < results.length; i++) {
        let tag_order = process.env.NODE_TAG_ORDER.replace(
          "sequential",
          "seq"
        ).split(/[ -]+/);
        let tag =
