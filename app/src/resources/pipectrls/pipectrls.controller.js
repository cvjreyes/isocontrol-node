...
const getPipesByStatus = async (req, res) => {
  //Get de las lineas en funcion de su status
  const tray = req.params.status;
  sql.query(
    "SELECT id FROM pestpipes WHERE name = ?",
    [tray],
    (err, results) => {
      if (!results[0]) {
  ...
  sql.query("SELECT id FROM users WHERE email = ?", [user], (err, results) => {
    ...
    if (!results[0]) {
      ...
      for (let i = 0; i < pipes.length; i++) {
        //Por cada linea seleccionada hacemos el claim
        sql.query(
          "UPDATE pipectrls SET claimed = 1, user_id = ? WHERE id = ?",
          [user_id, pipes[i]],
          (err, results) => {
            if (err) {
              console.log(err);
...
const unclaimPipes = async (req, res) => {
  //Unclaim de una linea
  const pipes = req.body.pipes;

  for (let i = 0; i < pipes.length; i++) {
    sql.query(
      "UPDATE pipectrls SET claimed = 0, user_id = null WHERE id = ?",
      [pipes[i][0]],
      (err, results) => {
        if (err) {
          console.log(err);
...
const pipingMyTray = async (req, res) => {
  //Get de las lineas reclamadas por un user
  ...
  sql.query("SELECT id FROM users WHERE email = ?", [email], (err, results) => {
    ...
    if (!results[0]) {
      ...
      sql.query(
        "SELECT DISTINCT pipectrls.id, pipectrls.tag, pipectrls.status_id, pestpipes.name as tray, next_status.name as next, pestpipes.progress, pestpipes.stage1, pipectrls.valves, pipectrls.instruments, tpipes.code, pipectrls.updated_at, users.name, CASE WHEN misoctrls.id IS NULL THEN 'Not in IsoTracker' ELSE 'In IsoTracker' END AS isotracker FROM pipectrls LEFT JOIN dpipes_view ON pipectrls.tag = dpipes_view.tag LEFT JOIN misoctrls ON misoctrls.isoid COLLATE utf8mb4_unicode_ci = dpipes_view.isoid LEFT JOIN tpipes ON dpipes_view.tpipes_id = tpipes.id LEFT JOIN users ON pipectrls.user_id = users.id LEFT JOIN pestpipes ON pipectrls.status_id = pestpipes.id LEFT JOIN pestpipes next_status ON (pipectrls.status_id + 1) = next_status.id  WHERE users.id = ? AND pipectrls.`deleted` is null",
        [user_id],
        (err, results) => {
          if (!results[0]) {
...
const nextStep = async (req, res) => {
  //Avanzamos las lineas a la siguiente bandeja
  const pipes = req.body.pipes;
  for (let i = 0; i < pipes.length; i++) {
    //Por cada linea
    //Obtenemos su status
    sql.query(
      "SELECT status_id, code FROM pipectrls LEFT JOIN dpipes ON pipectrls.tag = dpipes.tag LEFT JOIN tpipes ON dpipes.tpipes_id = tpipes.id WHERE pipectrls.id = ?",
      [pipes[i]],
      (err, results) => {
        if (!results[0]) {
          res.send({ success: false }).status(401);
          ...
          sql.query(
            "UPDATE pipectrls SET claimed = null, user_id = null, status_id = ? WHERE id = ?",
            [next_id, pipes[i]],
            (err, results) => {
              if (err) {
                console.log(err);
...
const sendValves = async (req, res) => {
  //Marcar que una linea tiene valvulas
  const pipe_id = req.body.id;
  sql.query(
    "UPDATE pipectrls SET valves = 1 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const sendInstruments = async (req, res) => {
  //Marcar que una linea tiene instrumentos
  const pipe_id = req.body.id;

  sql.query(
    "UPDATE pipectrls SET instruments = 1 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const sendNA = async (req, res) => {
  //Marcar que una linea no requiere ni de valvulas ni de instrumentos
  const pipe_id = req.body.id;

  sql.query(
    "UPDATE pipectrls SET valves = 2, instruments = 2 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const cancelValves = async (req, res) => {
  //Cancelar el requisito de valvulas de una linea
  const pipe_id = req.body.id;
  sql.query(
    "UPDATE pipectrls SET valves = 0 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const cancelInstruments = async (req, res) => {
  //Cancelar el requisito de instrumentos de una linea
  const pipe_id = req.body.id;
  sql.query(
    "UPDATE pipectrls SET instruments = 0 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const cancelNA = async (req, res) => {
  //Cancelar la indicacion de que una linea no requiere de valvulas ni instrumentacion
  const pipe_id = req.body.id;
  sql.query(
    "UPDATE pipectrls SET valves = 0, instruments = 0 WHERE id = ?",
    [pipe_id],
    (err, results) => {
      if (err) {
        console.log(err);
...
const returnPipes = async (req, res) => {
  //Retornar una linea a su bandeja anterior. Funciona igual que sendPipes pero hacia atras
  const pipes = req.body.pipes;
  for (let i = 0; i < pipes.length; i++) {
    sql.query(
      "SELECT status_id, code FROM pipectrls LEFT JOIN dpipes ON pipectrls.tag = dpipes.tag LEFT JOIN tpipes ON dpipes.tpipes_id = tpipes.id WHERE pipectrls.id = ?",
...
const getDeletedPipes = async (req, res) => {
  //Select de las lineas que han sido eliminadas
  sql.query(
    "SELECT pipectrls.id, pipectrls.tag, pipectrls.status_id, tpipes.code, pipectrls.updated_at, users.name FROM pipectrls LEFT JOIN dpipes ON pipectrls.tag = dpipes.tag LEFT JOIN tpipes ON dpipes.tpipes_id = tpipes.id LEFT JOIN users ON pipectrls.user_id = users.id WHERE `deleted` = 1",
    (err, results) => {
      if (!results[0]) {
        res.send({ rows: [] }).status(401);
      } else {
...
const deletePipes = async (req, res) => {
  ...
  for (let i = 0; i < pipes.length; i++) {
    sql.query(
      "UPDATE pipectrls SET deleted = 1 WHERE id = ?",
      [pipes[i]],
      (err, results) => {
        if (err) {
          console.log(err);
...
const restorePipes = async (req, res) => {
  //Restaurar una linea eliminada
  const pipes = req.body.pipes;
  for (let i = 0; i < pipes.length; i++) {
    sql.query(
      "UPDATE pipectrls SET deleted = null WHERE id = ?",
      [pipes[i]],
      (err, results) => {
        if (err) {
          console.log(err);
  ...
  await sql.query(
    ...
    (err, results) => {
      if (results[0]) {
        for (let i = 0; i < results.length; i++) {
          if (
            results[i].calc_notes != "NA" &&
            results[i].calc_notes != "unset"
          ) {
            estimated_weight += 10;
          } else {
            if (
              (process.env.NODE_MMDN == 1 && results[i].diameter < 2.0) ||
              (process.env.NODE_MMDN == 0 && results[i].diameter < 50)
            ) {
              estimated_weight += 3;
            } else {
              estimated_weight += 5;
      ...
      sql.query(
        "SELECT SUM(stage1_weight) as weight FROM dpipes LEFT JOIN tpipes ON dpipes.tpipes_id = tpipes.id",
        (err, results) => {
          if (results[0]) {
                ...
                (err, results) => {
                  ...
                  res
                    .send({
                      ...
                      modelledWeight: modelled_weight,
                      progress: progress,
                    })
                    .status(200);
                }
...
const estimatedPipingCustomWeight = async (req, res) => {
  //Lo mismo pero para el caso en el que se pueden forzar los status (en ese caso vienen de otra tabla)
  let estimated_weight = 0,
    modelled_weight = 0,
    progress = 0;
  await sql.query(
...
const isocontrolProgress = async (req, res) => {
  //Get de progreso real de isocontrol
...
const isocontrolEstimated = async (req, res) => {
  //Get de progreso estimado de isocontrol
...
const isocontrolForecast = async (req, res) => {
  //Get del foreacast de isocontrol
...
const submitEstimatedForecastIFD = async (req, res) => {
  //Submit del forecast y la estimacion
  ...
  for (let i = 0; i < estimated.length; i++) {
    ...
    sql.query(
      "UPDATE epipes_ifd SET progress = ? WHERE week = ?",
      [estimated[i], i + 1],
      (err, results) => {
        if (err) {
          console.log(err);
  ...
  for (let i = 1; i + 1 < forecast.length; i++) {
    ...
    sql.query(
      "UPDATE forecast_ifd SET progress = ? WHERE week = ?",
      [forecast[i], i + 1],
      (err, results) => {
        if (err) {
          console.log(err);
...
cron.schedule("0 0 * * 0", async () => {
  //Se guarda periodicamente el progreso de isocontrol
  let estimated_weight = 0,
    modelled_weight = 0,
    progress = 0;
  //Obtenemos el peso de las lineas estimadas
  await sql.query(
    "SELECT diameter, calc_notes FROM estimated_pipes LEFT JOIN `lines` on estimated_pipes.line_refno = `lines`.refno",
    (err, results) => {
      if (results[0]) {
        for (let i = 0; i < results.length; i++) {
          if (
        ...
        (err, results) => {
          ...
          sql.query(
            ...
            (err, results) => {
              ...
              sql.query(
                "SELECT `week` FROM pisocontrol ORDER BY `week` DESC LIMIT 1",
                (err, results) => {
                  let week = 1;
                  if (!results[0]) {
                  } else {
                    ...
                    sql.query(
                      "INSERT INTO pisocontrol(week, progress) VALUES(?,?)",
                      [week, progress],
                      (err, results) => {
                        if (err) {
                          console.log(err);
