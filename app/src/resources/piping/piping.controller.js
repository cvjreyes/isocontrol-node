...
const pipingEstimated = (req, res) => {
  sql.query(
    "SELECT epipes.id, areas.name as area, tpipes.name as type, epipes.qty as quantity FROM epipes JOIN areas ON epipes.areas_id = areas.id JOIN tpipes ON epipes.tpipes_id = tpipes.id",
    (err, results) => {
      res
        .json({
          rows: results,
        ...
        (err, results) => {
          ...
          sql.query(
            ...
            (err, results) => {
              ...
              sql.query(
                "SELECT * FROM epipes WHERE id = ?",
                [rows[i]["id"]],
                (err, results) => {
                  if (!results[0]) {
                    sql.query(
                      "INSERT INTO epipes(units_id, areas_id, tpipes_id, qty) VALUES(?,?,?,?)",
                    ...
                    sql.query(
                      "UPDATE epipes SET areas_id = ?, tpipes_id = ?, qty = ? WHERE id = ?",
...
const pipingWeight = async (req, res) => {
  sql.query(
    "SELECT qty, weight FROM epipes RIGHT JOIN tpipes ON epipes.tpipes_id = tpipes.id",
    (err, results) => {
