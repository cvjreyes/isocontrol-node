...
const gcurve = (req, res) => {
  sql.query(
    "SELECT gpipes.week as week, gpipes.progress as progress_pipes, gpipes.estimated as estimated_pipes, gequis.progress as progress_equis, gequis.estimated as estimated_equis, ginsts.progress as progress_insts, ginsts.estimated as estimated_insts, gelecs.progress as progress_elecs, gelecs.estimated as estimated_elecs, gcivils.progress as progress_civils, gcivils.estimated as estimated_civils FROM gpipes JOIN gequis on gpipes.id = gequis.id JOIN ginsts ON gpipes.id = ginsts.id JOIN gelecs ON gpipes.id = gelecs.id JOIN gcivils ON gpipes.id = gcivils.id",
    (err, results) => {
      if (err) {
        res.status(401);
      } else {
...
const submitEquipProgress = (req, res) => {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i]["Week"] || rows[i]["Week"] == "") {
      ...
      sql.query(
        "SELECT * FROM gequis WHERE id = ?",
        [rows[i]["id"]],
        (err, results) => {
          if (!results[0]) {
            sql.query(
              "INSERT INTO gequis(week, estimated) VALUES(?,?)",
              [rows[i]["Week"], rows[i]["Estimated"]],
              (err, results) => {
                if (err) {
                  console.log(err);
                  res.status(401);
            ...
            sql.query(
              "UPDATE gequis SET week = ?, estimated = ? WHERE id = ?",
              [rows[i]["Week"], rows[i]["Estimated"], rows[i]["id"]],
              (err, results) => {
                if (err) {
                  console.log(err);
                  res.status(401);
...
const submitInstProgress = (req, res) => {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i]["Week"] || rows[i]["Week"] == "") {
      ...
      sql.query(
        "SELECT * FROM ginsts WHERE id = ?",
        [rows[i]["id"]],
        (err, results) => {
          if (!results[0]) {
            sql.query(
              "INSERT INTO ginsts(week, estimated) VALUES(?,?)",
              [rows[i]["Week"], rows[i]["Estimated"]],
              (err, results) => {
                if (err) {
                  console.log(err);
            ...
            sql.query(
              "UPDATE ginsts SET week = ?, estimated = ? WHERE id = ?",
              [rows[i]["Week"], rows[i]["Estimated"], rows[i]["id"]],
              (err, results) => {
                if (err) {
                  console.log(err);
...
const submitCivilProgress = (req, res) => {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i]["Week"] || rows[i]["Week"] == "") {
      ...
      sql.query(
        "SELECT * FROM gcivils WHERE id = ?",
        [rows[i]["id"]],
        (err, results) => {
          if (!results[0]) {
            sql.query(
              "INSERT INTO gcivils(week, estimated) VALUES(?,?)",
              [rows[i]["Week"], rows[i]["Estimated"]],
              (err, results) => {
                if (err) {
                  console.log(err);
            ...
            sql.query(
              "UPDATE gcivils SET week = ?, estimated = ? WHERE id = ?",
              [rows[i]["Week"], rows[i]["Estimated"], rows[i]["id"]],
              (err, results) => {
                if (err) {
                  console.log(err);
...
const submitElecProgress = (req, res) => {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i]["Week"] || rows[i]["Week"] == "") {
      ...
      sql.query(
        "SELECT * FROM gelecs WHERE id = ?",
        [rows[i]["id"]],
        (err, results) => {
          if (!results[0]) {
            sql.query(
              "INSERT INTO gelecs(week, estimated) VALUES(?,?)",
              [rows[i]["Week"], rows[i]["Estimated"]],
              (err, results) => {
                if (err) {
                  console.log(err);
            ...
            sql.query(
              "UPDATE gelecs SET week = ?, estimated = ? WHERE id = ?",
              [rows[i]["Week"], rows[i]["Estimated"], rows[i]["id"]],
              (err, results) => {
                if (err) {
                  console.log(err);
...
const submitPipingProgress = (req, res) => {
  ...
  for (let i = 1; i < rows.length; i++) {
    if (!rows[i]["Week"] || rows[i]["Week"] == "") {
      ...
      sql.query(
        "SELECT * FROM gpipes WHERE id = ?",
        [rows[i]["id"]],
        (err, results) => {
          if (!results[0]) {
            sql.query(
              "INSERT INTO gpipes(week, estimated) VALUES(?,?)",
              [rows[i]["Week"], rows[i]["Estimated"]],
              (err, results) => {
                if (err) {
                  console.log(err);
            ...
            sql.query(
              "UPDATE gpipes SET week = ?, estimated = ? WHERE id = ?",
              [rows[i]["Week"], rows[i]["Estimated"], rows[i]["id"]],
              (err, results) => {
                if (err) {
                  console.log(err);
      ...
      sql.query(
        ...
        (req, results) => {
          ...
          if (process.env.NODE_PROGRESS_DIAMETER_FILTER) {
            let q1 =
              "SELECT COUNT(tpipes_id) FROM dpipes JOIN diameters ON dpipes.diameter_id = diameters.id WHERE tpipes_id = 1 AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
            let q2 =
              "SELECT COUNT(tpipes_id) FROM dpipes JOIN diameters ON dpipes.diameter_id = diameters.id WHERE tpipes_id = 2 AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
            let q3 =
              "SELECT COUNT(tpipes_id) FROM dpipes JOIN diameters ON dpipes.diameter_id = diameters.id WHERE tpipes_id = 3 AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
    ...
    (req, results) => {
      ...
      sql.query(
        ...
        (req, results) => {
          ...
          if (process.env.NODE_PROGRESS_DIAMETER_FILTER) {
            let q1 =
              "SELECT COUNT(tpipes_id) FROM dpipes_view INNER JOIN misoctrls ON dpipes_view.isoid COLLATE utf8mb4_unicode_ci = misoctrls.isoid JOIN diameters ON dpipes_view.diameter_id = diameters.id WHERE tpipes_id = 1 AND (revision = 0 OR (revision = 1 AND issued = 1)) AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
            let q2 =
              "SELECT COUNT(tpipes_id) FROM dpipes_view INNER JOIN misoctrls ON dpipes_view.isoid COLLATE utf8mb4_unicode_ci = misoctrls.isoid JOIN diameters ON dpipes_view.diameter_id = diameters.id WHERE tpipes_id = 2 AND (revision = 0 OR (revision = 1 AND issued = 1)) AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
            let q3 =
              "SELECT COUNT(tpipes_id) FROM dpipes_view INNER JOIN misoctrls ON dpipes_view.isoid COLLATE utf8mb4_unicode_ci = misoctrls.isoid JOIN diameters ON dpipes_view.diameter_id = diameters.id WHERE tpipes_id = 3 AND (revision = 0 OR (revision = 1 AND issued = 1)) AND dn " +
              process.env.NODE_PROGRESS_DIAMETER_FILTER;
            sql.query(q1, (err, results) => {
              const tp1 = results[0]["COUNT(tpipes_id)"];
              sql.query(q2, (err, results) => {
                ...
                sql.query(q3, (err, results) => {
                  ...
                  sql.query("SELECT weight FROM tpipes", (err, results) => {
                    ...
                    res
                      .json({
                        progressISO: ((progress / maxProgress) * 100).toFixed(
                          2
                        ),
                        realprogressISO: (
                          (realprogress / maxProgress) *
